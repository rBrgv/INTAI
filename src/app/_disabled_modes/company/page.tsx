...
export default function CompanyPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("companyModeDraft");
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem("companyModeDraft", JSON.stringify({
      jdText,
      topSkills,
      resumeText,
      questionCount,
  ...
  }, [jdText, topSkills, resumeText, questionCount, difficultyCurve, currentStep, completedSteps]);
  ...
  const handleNext = () => {
    ...
    if (currentStep === 1) {
      ...
      // Auto-populate skills if we have suggestions and no skills yet
      if (suggestedSkills.length > 0 && topSkills.filter(s => s.trim()).length === 0) {
        const skillsToAdd = suggestedSkills.slice(0, 5);
        setTopSkills(skillsToAdd.map(s => s));
      }
    }
  ...
  async function startInterview() {
    ...
    localStorage.removeItem("companyModeDraft");
  ...
  return (
    <Container className="py-8">
      ...
      <h2 className="text-2xl font-semibold text-[var(--text)] mb-2">Company Mode Setup</h2>
      <p className="text-[var(--muted)] mb-6">
        Configure your interview in 5 simple steps.
      </p>
      ...
      <Card className="app-card">
        ...
        {currentStep === 3 && (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-[var(--text)]">Step 3: Candidate Resume</h3>
            <FileUploadWithPreview
        ...
        {currentStep === 4 && (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-[var(--text)]">Step 4: Interview Setup</h3>
            <div>
        ...
        {currentStep === 5 && (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-[var(--text)]">Step 5: Review & Start</h3>
            <div className="space-y-3 text-sm bg-[var(--bg)] rounded-lg p-4">
              ...
              <div>
                ...
                <span className="text-[var(--muted)]">
                  {topSkills.filter(s => s.trim()).join(", ") || "None"}
                </span>
              </div>
              <div>
